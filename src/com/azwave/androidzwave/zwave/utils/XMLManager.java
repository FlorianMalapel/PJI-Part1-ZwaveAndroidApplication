...
public abstract class XMLManager {
	...
	protected static String path = "assets/";
	...
	public String getContentOfTheFile(String filePath){
	...
	public void readDeviceClasses(String location) {
		...
		try {
			...
			String content = getContentOfTheFile(location);
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				for (int j = 0; j < nodeListSpec.getLength(); j++) {
					...
					mappingspec = basicspec.length() > 0 ? Integer.decode(basicspec).byteValue() : 0;

					specificDeviceClasses.put(keyspec, new DeviceClass(mandatoryspec, mappingspec, labelspec));
				}

				genericDeviceClasses.put(key, new GenericDeviceClass(mandatory, mapping, label, specificDeviceClasses));
			}
			log.add(String.format("Reading 'device_classes.xml' (basics: %d, generics: %d)",
					basicDeviceClasses.size(), genericDeviceClasses.size()));
		} catch (Exception e) {
			log.add(String.format("Error while reading 'device_classes.xml': %s", e.toString()));
		}
	}
		...
		try {
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				for (int j = 0; j < nodeListProduct.getLength(); j++) {
					...
					typeProduct = Integer.decode("0x" + elemProduct.getAttribute("type")).shortValue();
					idProduct = Integer.decode("0x" + elemProduct.getAttribute("id")).shortValue();
					nameProduct = elemProduct.getAttribute("name");
					configProduct = elemProduct.getAttribute("config");

					Product product = new Product(id, typeProduct, idProduct,nameProduct, configProduct);
			...
			log.add(String.format("Reading 'manufacturer_specific.xml' (manufacturers: %d, products: %d)",
	...
	public boolean readProduct(com.azwave.androidzwave.zwave.nodes.Node node,
		...
		try {
			 ...
			 */
			Document doc = (Document) new InputSource(path + location);
			doc.getDocumentElement().normalize();
	...
	private void readDeviceProtocol(com.azwave.androidzwave.zwave.nodes.Node node, Document doc) throws Exception {
	...
	public void reloadConfigManufacturer(com.azwave.androidzwave.zwave.nodes.Node node) {
		if (node != null) {
