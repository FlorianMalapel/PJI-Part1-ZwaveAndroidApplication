...
public class SerialInputOutputManager implements Runnable {
    ...
    public void run() {
        ...
        try {
            while (true) {
                if (getState() != State.RUNNING) {
                	System.out.println("Stopping mState=" + getState());
                    break;
                }
                step();
            }
        } catch (Exception e) {
        	System.out.println("Run ending due to exception: " + e.getMessage());
    ...
    private void step() throws IOException {
        ...
        int len = -1;
		try {
			len = mDriver.read(mReadBuffer.array());
        ...
        if (outBuff != null) {
            if (DEBUG) {
            	System.out.println("Writing data len=" + len);
            }
            try {
				mDriver.write(outBuff);
