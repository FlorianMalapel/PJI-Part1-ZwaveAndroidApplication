...
public class Driver implements IOServiceListener {
	...
	public Driver(QueueManager queueManager, NodeManager nodeManager, XMLManager xmlmanager, UsbSerialDriver serialDriver, Log log) {
		this.log = log;
		...
		if (item.getCommand() == QueueCommand.QueryStageComplete) {
			...
			if (node != null) {
				if (stage != QueryStage.Complete) {
					if (!item.isRetry()) {
						log.add(String.format("W: [Node %d] Query Stage Complete --- %s", SafeCast.toInt(node.getNodeId()), stage.name()));
		...
		if (item.getCommand() == QueueCommand.Controller) {
			...
			if (item.getCci().isControllerCommandDone()) {
				...
				if (item.getCci().getControllerCallback() != null) {
					item.getCci().getControllerCallback().onAction(
	...
	public void writeMsg() throws IOException {
		...
		if (attemp >= msg.getMaxSendAttempts() || (node != null && !node.isNodeAlive() && !msg.isNoOperation())) {
	...
	public void writeMsg(Msg msg) throws IOException {
		try {
			serialDriver.write(msg.toArray());
		} catch (UsbNotActiveException | UsbNotClaimedException | UsbDisconnectedException | UsbException e) {
			e.printStackTrace();
		}
		log.add(String.format("W: [Node %d] %s", SafeCast.toInt(SafeCast.nodeIdFromMsg(msg)), msg.toString()));
		System.out.println("WriteMsg: OK");
	}
